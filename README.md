...
Отчет структурирован в два уровня:
- общая статистика за все время (для оценки общей результативности);
- подробный отчет за выбранный день (для оперативного контроля).
...
- Менеджерский обзор диалогов:
  - список диалогов за день с risk-флагами;
  - полный транскрипт по каждому лиду;
  - API-фильтр по проблемным диалогам (`risk_only=true`).
- Режимы запуска:
...
### 5) Проверка диалогов менеджером
- total dialogs for day
- risky dialogs count
- risk flags per dialog
- transcript with message timeline
...
- `GET /analytics/dialogs/daily?report_date=2026-03-12&risk_only=true&include_transcript=true&limit=20`
